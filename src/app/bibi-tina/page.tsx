...
export default function BibiTinaPage() {
    ...
    useEffect(() => {
        const fetchEpisodes = async () => {
            try {
                ...
                const results = await searchArtist('Bibi und Tina', 600);
                ...
                if (results && results.length > 0) {
                    ...
                    uniqueById.forEach(item => {
                        ...
                        // Exclusion list for spin-offs that use "Folge" but aren't main series classics
                        ...
                        if (match && !isSpinOff) {
                            ...
                            // Group all non-classics (including Weihnacht/Lieder) together
                            others.push(item);
                        }
                    });
    ...
    useEffect(() => {
        const isClassic = (name: string) => {
            ...
            // Disqualify spin-offs from being classified as a core Classic episode
            const isSpinOff = /Minis|Gute\s*-?\s*Nacht|Schäfchenwolken|Box/i.test(name);
